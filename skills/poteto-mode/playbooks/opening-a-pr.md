...
**Worktree.** Work from a git worktree off master; subagents inherit it. Multiple `task` items on the same branch each get their own worktree, or `git fetch && git reset --hard origin/<branch>` between them. Dirty branch with unrelated work: patch out, fresh worktree, apply. Snarled worktree: reset from master, redo minimally.
...
- `## Blast Radius`. State who and what the change touches. Explain why the change is safe or risky. If master is red without the fix, name the continuing cost.
