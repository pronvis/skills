...
- Exactly one stacker per stack may run `gt`, serialized within its stack; record the holder in the standing orders. Restacks run in the stacker's own worktree, never in the coordinator's checkout.
...
- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the agent's status from `hub` (`list`, `jobs`). Transcript mtime is not liveness.
