...
# Sessions dir: ~/.omp/agent/sessions/<encoded-cwd>. A cwd under $HOME encodes
# as -<relative> and anything else as --<absolute>--, with / replaced by -.
canon=$(cd "$main_wt" 2>/dev/null && pwd -P)
case "$canon" in
	"$HOME"/*) slug="-$(printf '%s' "${canon#"$HOME"/}" | tr / -)" ;;
	*) slug="--$(printf '%s' "${canon#/}" | tr / -)--" ;;
esac
