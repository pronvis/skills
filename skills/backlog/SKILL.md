...
ordinary output before calling `ask`. The question text names the item and carries the one-line
...
`/skill:backlog --all` walks every current item to a disposition. Check this mode before slug handling.
...
3. Take the items ONE AT A TIME: print that item's briefing, ask about it alone with **ask**
...
`/skill:backlog <slug>` names one item: `docs/backlog/<slug>.md`, the file name without its extension. Read
...
3. Report every item, one line each — `yes` first, then `later`, then `no`, oldest `added` first within
   ...
   its reasoning.
4. Offer concrete actions with **ask**, never prose, with a recommendation first:
...
step with **ask**, staging the exact paths this invocation created or edited and not the
