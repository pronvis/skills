...
Before spawning investigators, list the available MCPs from the omp environment. Use the available-tools map when present. Otherwise inspect the enabled MCP servers in omp's MCP config (`.omp/mcp.json` in the project, `~/.omp/agent/mcp.json` for the user, or the imported configs omp discovers from other tools).
...
Subagent config (each `task` item):
- `agent`: `task` (default writing agent type; it runs on `task.agentModelOverrides.task` when set, otherwise the parent session's model; your why-investigators tier default is `@sonnet`)
- full tool set. **Do not dispatch a read-only agent type.** It strips MCP access, which disables MCP-backed investigators entirely. The source control investigator would be safe on a read-only agent type, but keep agent types uniform. Investigators still shouldn't write anything. That's a posture, not a sandbox.
...
Spawn one synthesizer as a `task` item:

- `agent`: `poteto-runner-fable` (writing agent type on your why-synthesizer tier, default `@fable`)
- full tool set. The synthesizer's quality check spot-verifies citations, which can require MCP access. A read-only agent type strips MCPs and defeats that.
